...
TRANSLATIONS = {
    "中文": {
        ...
        "uploader": "上传 Excel / 文本 / PNG 相图（可多选）",
        "active_file": "当前处理文件",
        ...
        "png_upload_hint": "PNG 相图建议使用红色=液体/水相、黄色=固体、白色=外部背景。Agent 会保留原图，并可标准化边缘相色、裁剪白边，再运行 2D 网格划分、衰减求解、T2 反演和可选 T2-T2 图。",
        "diagnosis": "### AI 数据诊断结果",
...
- 你也可以上传红黄白 PNG 相图，Agent 会保留原图，并可标准化边缘颜色、自动裁剪白边后运行二维 NMR 模拟。
- 你也可以上传 Excel、CSV 或支持的文本数据，Agent 会先检查时间列和信号列。
- 如果你不懂平滑因子，默认建议用 L-curve 自动选择；如果你指定固定值，也可以复现实验设置。
- 生成的几何预览、网格、衰减曲线、T2 谱、可选 T2-T2 图、拟合表、分峰表、图像和报告会出现在右侧产物栏。
...
- 结果是数据处理和解释辅助，不替代你的样品背景和专业判断。""",
    ...
    "English": {
        ...
        "uploader": "Upload Excel / text / PNG phase maps",
        ...
        "png_upload_hint": "PNG phase maps should use red=liquid/water, yellow=solid, and white=outside background. The agent can preserve the original image, normalize near-edge phase colors, crop white margins, then run 2D meshing, decay solving, T2 inversion, and optional reduced T2-T2 mapping.",
...
- Or upload red/yellow/white PNG phase maps; the agent can preserve the original image, normalize edge colors, crop white margins, and run phase-map-driven 2D NMR simulation.
...
- Generated geometry previews, meshes, decay curves, T2 spectra, optional T2-T2 maps, fit tables, peak tables, figures, and reports appear in the artifact rail.
