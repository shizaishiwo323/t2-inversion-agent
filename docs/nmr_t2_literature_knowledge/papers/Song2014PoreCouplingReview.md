---
citation_key: Song2014PoreCouplingReview
...
pages: 5
text_cache: "_extracted_text\Song2014_5e544615053149c7.txt"
---

# Song2014PoreCouplingReview

- **题名/文件**：Song2014_reviewLetters (`Song2014_reviewLetters.pdf`)
- **建议引用格式**：`[@Song2014PoreCouplingReview]`
- **原始来源**：`NMR application\Lab NMR\Pore coupling\Song2014_reviewLetters.pdf`
- **入选理由**：孔耦合/多峰谱解释的短文背景，适合结果讨论引用。
...
- **可写入报告的引用句式**：峰合并或谱形变宽不一定说明只有一个孔群，也可能是孔间扩散耦合把多个弛豫环境混合后的结果。 [@Song2014PoreCouplingReview]
...
- **可写入报告的引用句式**：复杂样品的一维 T2 峰不总能唯一分配给物理孔群，多维 NMR 可作为验证和补充约束。 [@Song2014PoreCouplingReview]
- **证据定位**：见下方证据锚点；若没有页码锚点，则由题名、入选理由和文本关键词归类，正式引用前需复核原 PDF。
...
- **可写入报告的引用句式**：短 T2 组分通常支持更强表面弛豫、更小孔径或更高表面积/体积比的解释，但需要结合表面弛豫率和扩散区制判断。 [@Song2014PoreCouplingReview]
- **证据定位**：见下方证据锚点；若没有页码锚点，则由题名、入选理由和文本关键词归类，正式引用前需复核原 PDF。
...
- 主要证据主题：扩散耦合与孔间交换；T2-T2/T1-T2 多维 NMR 与交换；表面弛豫、孔径与短 T2。
