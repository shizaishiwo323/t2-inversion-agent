...
pages: 8
text_cache: "_extracted_text\X2016_0ead8882f158f8d2.txt"
---

# Song2016T2T2

- **题名/文件**：The robust identification of exchange from T2â€“T2 time-domain features (`x_Song2016.pdf`)
- **建议引用格式**：`[@Song2016T2T2]`
- **原始来源**：`NMR measurement and modeling\T2-T2\x_Song2016.pdf`
- **入选理由**：二维 NMR/孔隙流体解释背景，适合扩展结果讨论。
...
- **可写入报告的引用句式**：复杂样品的一维 T2 峰不总能唯一分配给物理孔群，多维 NMR 可作为验证和补充约束。 [@Song2016T2T2]
...
- **可写入报告的引用句式**：峰合并或谱形变宽不一定说明只有一个孔群，也可能是孔间扩散耦合把多个弛豫环境混合后的结果。 [@Song2016T2T2]
- **证据定位**：见下方证据锚点；若没有页码锚点，则由题名、入选理由和文本关键词归类，正式引用前需复核原 PDF。
...
- **可写入报告的引用句式**：T2 谱不是衰减曲线的唯一直接读数，而是受噪声、采样窗口和正则化约束影响的反演结果。 [@Song2016T2T2]
- **证据定位**：见下方证据锚点；若没有页码锚点，则由题名、入选理由和文本关键词归类，正式引用前需复核原 PDF。
