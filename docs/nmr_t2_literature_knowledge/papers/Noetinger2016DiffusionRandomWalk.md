---
citation_key: Noetinger2016DiffusionRandomWalk
...
pages: 45
text_cache: "_extracted_text\Noetinger2016_184a48b928044e82.txt"
---

# Noetinger2016DiffusionRandomWalk

- **题名/文件**：Noetinger2016_preprint (`Noetinger2016_preprint.pdf`)
- **建议引用格式**：`[@Noetinger2016DiffusionRandomWalk]`
- **原始来源**：`NMR simulation\Random Walk\Noetinger2016_preprint.pdf`
- **入选理由**：扩散/随机游走理论背景较强，适合作为深入理论补充。
...
- **可写入报告的引用句式**：随机游走正演提供了从孔隙几何和物理参数到衰减曲线/T2 谱的机制桥梁。 [@Noetinger2016DiffusionRandomWalk]
...
- **可写入报告的引用句式**：T2 响应携带孔域扩散和边界弛豫共同作用后的有效模态信息，而不只是几何孔径本身。 [@Noetinger2016DiffusionRandomWalk]
- **证据定位**：见下方证据锚点；若没有页码锚点，则由题名、入选理由和文本关键词归类，正式引用前需复核原 PDF。
