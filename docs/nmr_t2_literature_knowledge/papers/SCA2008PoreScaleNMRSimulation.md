---
citation_key: SCA2008PoreScaleNMRSimulation
...
pages: 12
text_cache: "_extracted_text\Sca2008_8c036f760bd71b44.txt"
---

# SCA2008PoreScaleNMRSimulation

- **题名/文件**：2008: Pore-Scale Simulation of Nmr Response in Carbonates (`SCA2008-30.pdf`)
- **建议引用格式**：`[@SCA2008PoreScaleNMRSimulation]`
- **原始来源**：`NMR simulation\Random Walk\SCA2008-30.pdf`
- **入选理由**：孔尺度/随机游走模拟相关会议论文，作为方法补充。
...
- **可写入报告的引用句式**：随机游走正演提供了从孔隙几何和物理参数到衰减曲线/T2 谱的机制桥梁。 [@SCA2008PoreScaleNMRSimulation]
...
- **可写入报告的引用句式**：短 T2 组分通常支持更强表面弛豫、更小孔径或更高表面积/体积比的解释，但需要结合表面弛豫率和扩散区制判断。 [@SCA2008PoreScaleNMRSimulation]
- **证据定位**：见下方证据锚点；若没有页码锚点，则由题名、入选理由和文本关键词归类，正式引用前需复核原 PDF。

### 受限扩散、几何与特征模态
- **什么结果时调用**：当几何改变导致 T2 峰位/峰宽改变，或圆孔、三角孔、复杂连通孔的结果不能用单一孔径解释时。
- **物理/数学机制**：有限孔域中的扩散-弛豫可看作边界条件控制的模态衰减问题；孔形、连通性和边界表面弛豫改变特征模态及有效弛豫时间。
- **可写入报告的引用句式**：T2 响应携带孔域扩散和边界弛豫共同作用后的有效模态信息，而不只是几何孔径本身。 [@SCA2008PoreScaleNMRSimulation]
- **证据定位**：见下方证据锚点；若没有页码锚点，则由题名、入选理由和文本关键词归类，正式引用前需复核原 PDF。
...
- 主要证据主题：随机游走/Monte Carlo 正演模拟；表面弛豫、孔径与短 T2；受限扩散、几何与特征模态。
