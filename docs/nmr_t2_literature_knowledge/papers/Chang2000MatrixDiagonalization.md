---
citation_key: Chang2000MatrixDiagonalization
...
pages: 12
text_cache: "_extracted_text\Chang2000_f12fea2e58027253.txt"
---

# Chang2000MatrixDiagonalization

- **题名/文件**：Ioannidis.PDF (`Chang2000.pdf`)
- **建议引用格式**：`[@Chang2000MatrixDiagonalization]`
- **原始来源**：`NMR simulation\matrix diagonalization methods\Chang2000.pdf`
- **入选理由**：矩阵对角化/特征模态方法相关，支撑解析/半解析正演理解。
...
- **可写入报告的引用句式**：多指数衰减可理解为扩散-弛豫系统多个特征模态叠加，而不仅是若干孤立孔群的简单相加。 [@Chang2000MatrixDiagonalization]
...
- **可写入报告的引用句式**：T2 响应携带孔域扩散和边界弛豫共同作用后的有效模态信息，而不只是几何孔径本身。 [@Chang2000MatrixDiagonalization]
- **证据定位**：见下方证据锚点；若没有页码锚点，则由题名、入选理由和文本关键词归类，正式引用前需复核原 PDF。
...
- **可写入报告的引用句式**：短 T2 组分通常支持更强表面弛豫、更小孔径或更高表面积/体积比的解释，但需要结合表面弛豫率和扩散区制判断。 [@Chang2000MatrixDiagonalization]
- **证据定位**：见下方证据锚点；若没有页码锚点，则由题名、入选理由和文本关键词归类，正式引用前需复核原 PDF。
...
- 主要证据主题：矩阵对角化/特征模态方法；受限扩散、几何与特征模态；表面弛豫、孔径与短 T2。
