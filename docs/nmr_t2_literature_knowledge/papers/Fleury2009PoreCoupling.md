---
citation_key: Fleury2009PoreCoupling
...
pages: 10
text_cache: "_extracted_text\X2009_384eac4242929edc.txt"
---

# Fleury2009PoreCoupling

- **题名/文件**：Quantitative analysis of diffusional pore coupling from T2-store-T2 NMR experiments (`x_Fleury2009.pdf`)
- **建议引用格式**：`[@Fleury2009PoreCoupling]`
- **原始来源**：`NMR application\Lab NMR\Pore coupling\x_Fleury2009.pdf`
- **入选理由**：孔耦合与 T2 分布解释，作为模拟结果和实验现象之间的桥梁。
...
- **可写入报告的引用句式**：峰合并或谱形变宽不一定说明只有一个孔群，也可能是孔间扩散耦合把多个弛豫环境混合后的结果。 [@Fleury2009PoreCoupling]
...
- **可写入报告的引用句式**：短 T2 组分通常支持更强表面弛豫、更小孔径或更高表面积/体积比的解释，但需要结合表面弛豫率和扩散区制判断。 [@Fleury2009PoreCoupling]
- **证据定位**：见下方证据锚点；若没有页码锚点，则由题名、入选理由和文本关键词归类，正式引用前需复核原 PDF。

## 检索关键词

`diffusive_coupling`, `exchange`, `peak_merging`, `pore_coupling`, `pore_coupling_t2t2_interpretation`, `pore_size`, `short_t2`, `surface_relaxation`, `surface_relaxivity`

## 证据锚点（非原文长引）

- 摘要可抽取：是；结论/总结可抽取：是。
- 本卡片只保留机制级释义和检索标签；需要逐字引用或页码时，请打开对应 PDF 或 `_extracted_text` 缓存复核。
- 主要证据主题：扩散耦合与孔间交换；表面弛豫、孔径与短 T2。
